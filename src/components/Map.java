...
public class Map implements Utilities {
	...
	public Map(int junctionsNum,String name) {
		junctions=new ArrayList<Junction>();
		roads=new ArrayList<Road>();
		lights=new ArrayList<TrafficLights>();
		System.out.println("\n================= CREATING JUNCTIONS=================");
		...
		if(name=="City") {
			...
			ArrayList<Integer> arr = getRandomIntArray(0,junctions.size()-1,getRandomInt(2,junctions.size()-1));
			for (int i=0; i<junctions.size();i++) {
				for (int j=0; j<junctions.size();j++) {
					if(i==j || arr.contains(i) || arr.contains(j)) {
						continue;
					}
