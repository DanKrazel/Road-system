...
public class Vehicle extends Point implements Utilities, Timer, Runnable,Cloneable { 
	private int id;
	...
	public Vehicle (Road currentLocation) {// random constructor
		...
		int x=(100+getRandomInt(1,50))/100;
		speed=vehicleType.getAverageSpeed()*x;
	}
	
	public Vehicle (Road currentLocation,String name) {// random constructor
		id=objectsCount++;
		//setDrive(new Driver());
		drive = new Driver();
		if(name=="City") {
			vehicleType=currentLocation.getVehicleTypes()[getRandomInt(0,currentLocation.getVehicleTypes().length-1)];
			while (vehicleType==Factory.getFactory(10).getVehicle("work") || vehicleType==Factory.getFactory(4).getVehicle("work") )
			{
				vehicleType=currentLocation.getVehicleTypes()[getRandomInt(0,currentLocation.getVehicleTypes().length-1)];
			}
			int x=(100+getRandomInt(1,30))/100;
			speed=vehicleType.getAverageSpeed()*x;
		}
		else if(name=="Country") {
			vehicleType=currentLocation.getVehicleTypes()[getRandomInt(0,currentLocation.getVehicleTypes().length-1)];
			while (vehicleType==Factory.getFactory(2).getVehicle("slow") || vehicleType==Factory.getFactory(10).getVehicle("public") )
			{
				vehicleType=currentLocation.getVehicleTypes()[getRandomInt(0,currentLocation.getVehicleTypes().length-1)];
			}
			int x=(100+getRandomInt(1,50))/100;
	...
	public void calcPositionOnRoad() {
		...
		int speed1=Math.min(road.getMaxSpeed(), getVehicleType().getAverageSpeed());
		...
		double distance=timeOnCurrentPart*speed1/10;
