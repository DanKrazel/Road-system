...
public class Driving implements Utilities, Timer, Runnable{
	...
	public Driving(int junctionsNum, int numOfVehicles,String name) {
		vehicles=new ArrayList<Vehicle>();
		allTimedElements=new ArrayList<Timer>();
		drivingTime=0;
		map=new Map(junctionsNum,name);
		System.out.println("\n================= CREATING VEHICLES =================");
		while(vehicles.size()<numOfVehicles) {
			Road temp=map.getRoads().get(getRandomInt(0,map.getRoads().size()));//random road from the map
			if(temp.getEnabled())
				vehicles.add(new Vehicle(temp,name));
			/*else if(temp.getEnabled()){
				ShapeCache.loadCache(vehicles.get(0));
				Vehicle clone=(Vehicle) ShapeCache.getShape(vehicles.get(0).getID());
	...
	/*public Driving(int junctionsNum,String name) {
		vehicles=new ArrayList<Vehicle>();
		allTimedElements=new ArrayList<Timer>();
		drivingTime=0;
		map=new Map(junctionsNum,name);
